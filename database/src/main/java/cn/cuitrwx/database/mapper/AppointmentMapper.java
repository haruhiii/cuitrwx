...
public interface AppointmentMapper {
    ...
    //获取所有预约信息
    @Select("select * from appointment  order by id  desc  limit #{start},#{total} order by id desc")
    List<Appointment> getAppointments(Integer start,Integer total);

    //通过用户编号 获取预约信息
    @Select("select * from appointment where openid = #{openid}  order by id desc")
    List<Appointment> getAppointmentsByOpenid(String openid);
    
    //新建预约
    @Insert("insert into appointment(openid, matter,comment,request_date) values (#{openid},#{matter},#{comment},#{requestDate})")
